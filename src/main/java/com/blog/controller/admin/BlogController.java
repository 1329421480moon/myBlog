...
public class BlogController {
    ...
    public String blogs(@RequestParam(required = false, defaultValue = "1", value = "pagenum") int pagenum, Model model) {
        ...
        log.info(pageInfo.getList().get(0).toString());
        model.addAttribute("pageInfo", pageInfo);
        setTypeAndTag(model);  //查询类型和标签
        return "admin/blogs";
    }

    @GetMapping("/blogs/search")  //按条件查询博客
    public String searchBlogs(BlogQuery blogQuery, HttpSession session,
    ...
    public String toAddBlog(Model model) {
        //返回一个blog对象给前端th:object（新增和修改是同一个页面,避免为空出现错误）
    ...
    public String addBlog(Blog blog, HttpSession session, RedirectAttributes attributes) {
        ...
        if (blog.getTypeId() == null || "".equals(blog.getTypeId())) {
            System.out.println("没有取到blog.getTypeId()");
        }
        //设置user属性
